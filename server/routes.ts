...
export async function registerRoutes(app: Express): Promise<Server> {
        ...
        const documents = await Promise.all(
          files.map(async (file) => {
            const document = await storage.createDocument({
              ...
              filePath: file.path,
              contentText:
                req.body.content || "Document content would be extracted here",
            });
  ...
  app.get(
    ...
    async (req: Request, res: Response) => {
      try {
        const documentId = parseInt(req.params.id);
        const analysisResult = await storage.getDocumentAnalysisResult(
          documentId
        );
      ...
      try {
        ...
        if (!lastUserMessage || !document) {
          return res
            .status(400)
            .json({
              message: "No message to respond to or document not found",
            });
        }
...
async function generateAIResponse(
  ...
  if (lowerMessage.includes("impact") || lowerMessage.includes("analysis")) {
    return `I've analyzed the impact of "${
      document.title
    }" against the knowledge base. The impact level is ${
      analysis?.impactLevel || "Medium"
    } with ${
      analysis?.impactedAreas?.length || 0
    } potential conflicts identified. Would you like to explore specific areas of impact?`;
  }
